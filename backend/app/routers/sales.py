...
def list_sales(db: Session = Depends(get_db), current_user: models.Usuario = Depends(get_current_user)):
    if current_user.rol not in ["admin", "seller"]:
        raise HTTPException(status_code=403, detail="No tienes permiso para ver el historial de ventas")
...
def my_purchases(db: Session = Depends(get_db), current_user: models.Usuario = Depends(get_current_user)):
    if current_user.rol != "buyer":
        raise HTTPException(status_code=403, detail="Solo los compradores pueden ver este historial")
